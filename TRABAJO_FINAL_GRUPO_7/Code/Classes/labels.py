...
class OverheadLabels:
    ...
    def info_labels(self):
        ...
        self.create_label(self.char_label, str.upper(c.NOMBRE), 75, 30)
    ...
    def load_screen_labels3(self):
        ...
        self.create_label(world_label3, 'LEVEL', 310, 200)
        self.create_label(number_label3, '3', 460, 200)

        self.center_labels3 = [world_label3, number_label3]
    ...
    def handle_level_state(self, level_labels):
        ...
        elif self.state == c.LOAD_SCREEN3:
            self.score = level_labels[c.SCORE]
            self.update_score_images(self.score_images, self.score)
            self.update_coin_total(level_labels)
    ...
    def input_box_labels(self):

        """
        Create labels for the INPUT BOX screen/state
        """

        nombre = []
        self.create_label(nombre, 'INGRESAR NOMBRE', 250, 200)
        self.input_box_labels = [nombre]
    ...
    def draw(self, surface):
        ...
        elif self.state == c.INPUT_BOX:
            self.draw_input_box_labels(surface)
        elif self.state == c.LOAD_SCREEN:
            self.draw_loading_screen_labels(surface)
        elif self.state == c.LOAD_SCREEN2:
            self.draw_loading_screen_labels2(surface)
        elif self.state == c.LOAD_SCREEN3:
            self.draw_loading_screen_labels3(surface)
        elif self.state == c.LEVEL:
    ...
    def draw_input_box_labels(self, surface):

        """
        Draws labels for input box state
        """

        for label in self.input_box_labels:
            for letter in label:
                surface.blit(letter.image, letter.rect)
    ...
    def draw_loading_screen_labels3(self, surface):

        """
        Draws labels for loading screen state
        """

        for word in self.center_labels3:
            for letter in word:
                surface.blit(letter.image, letter.rect)
...
class OverheadLabels3(OverheadLabels):
    ...
    def info_labels(self):
        ...
        self.create_label(self.world_label, 'LEVEL', 450, 30)
        self.create_label(self.time_label, 'TIME', 625, 30)
        self.create_label(self.stage_label, '3', 492, 55)
